...
app.get('/home', (req, res) => {
    if (req.session.loggedin) {
        fs.readFile(__dirname + '/data.json', 'utf8', (err, data) => {
            ...
            res.render(__dirname + '/view/home.ejs', {username: req.session.username, data: jsonData});
        })
    } else {
        res.redirect('/');
    }
...
app.get('/publier', (req, res) => {
    if (req.session.loggedin) {
        res.render(__dirname + '/view/publier.ejs', {username: req.session.username});
    } else {
        res.redirect('/');
    }
});
...
app.post('/uploadFiles',upload.array('media'), (req, res) => {
    ...
    fs.readFile(__dirname + '/data.json', 'utf8', (err, data) => {
        ...
        req.files.forEach(file => {
            fs.rename(file.path, file.path + '.' + file.mimetype.split('/')[1], (err) => {
                if (err) {
                    console.log(err);
                    return;
                }
...
app.get('/detail/:id', (req, res) => {
    if (req.session.loggedin) {
        fs.readFile(__dirname + '/data.json', 'utf8', (err, data) => {
            ...
            res.render(__dirname + '/view/detail.ejs', {username: req.session.username, item: item});
        })
    } else {
        res.redirect('/');
    }
